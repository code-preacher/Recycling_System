...
// Inserting
	...
	public function insertLocation($post)
	{
		...
		$sql = $this->con->query($query);
		if ($sql==true) {
			header("Location:view-location.php?msg=Location was created successfully&type=success");
		}else{
			header("Location:view-location.php?msg=Error adding data try again!&type=error");
		}
	}



	public function insertLocationCharge($post)
	{
		$location_id=$this->cleanse($post['location']);
		$type=strtoupper($this->cleanse($post['type']));
		$charge=$this->cleanse($post['charge']);
		$query="insert into location_charge(location_id,waste_type,charge) values('$location_id','$type','$charge')";
		$sql = $this->con->query($query);
		if ($sql==true) {
			header("Location:view-location-charge.php?msg=Location Charge was created successfully&type=success");
		}else{
			header("Location:view-location-charge.php?msg=Error adding data try again!&type=error");
		}
	}
